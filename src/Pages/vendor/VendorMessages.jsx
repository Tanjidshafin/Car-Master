...
import { ChatLayoutSkeleton, ChatThreadSkeleton, EmptyState, SectionHeading, SurfaceCard } from "../../Components/dashboard/PremiumShell"
  ...
  return (
    <div className="grid gap-4 md:grid-cols-3">
      <SurfaceCard className="md:col-span-1">
        <SectionHeading title="Buyer inbox" description="Keep high-intent prospects warm with quick, polished replies." />
        <ChatList
      ...
      <SurfaceCard className="hidden md:col-span-2 md:block">
        <SectionHeading
          title={activeConversation ? activeConversation.counterpart?.displayName || activeConversation.car_name : "Conversations"}
          description={activeConversation ? `Conversation about ${activeConversation.car_name}. Latest replies stay pinned at the top.` : "Select a chat to get detailed information."}
        ...
        />
        {!activeConversation ? (
          <EmptyState title="Select a chat" description="Select a chat to get detailed information and continue the buyer conversation." />
        ) : isMessagesLoading ? (
          <ChatThreadSkeleton />
        ) : (
          <>
